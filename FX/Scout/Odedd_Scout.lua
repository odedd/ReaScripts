    ...
    if r.GetExtState(Scr.ext_name, 'RUNNING') ~= 'TRUE' and OD_PrereqsOK({
        ...
        app.flow = {
            ...
            getActiveFilters = function()
                ...
                if app.temp.filter == nil then app.temp.filter = {} end
                ...
                for i, filterKey in pairs(FILTER_TYPES) do
                    ...
                    if filterKey ~= FILTER_TYPES.TAG then
                        ...
                        for tagKey, tagValue in pairs(app.temp.filter.tags or {}) do
        ...
        app.draw = {
            ...
            search = function(ctx)
                ...
                local drawResultsTable = function()
                    ...
                    local searchResultsH = select(2, ImGui.GetContentRegionAvail(ctx)) - fontLineFullHeight
                    ...
                    local handleScrolling = function()
                        ...
                        if app.temp.scrollIfNeeded then
                            ...
                            if selectedRow then
                                ...
                                local rowPosition = (selectedRow.totalIndex - 1) * fontLineFullHeight + 2
                    ...
                    if ImGui.BeginChild(ctx, 'resultsArea', w - (app.settings.current.showSideBar and (sideBarW + paddingX * 2) or 0) - (app.settings.current.showQuickChain and (quickChainW + paddingX * 2) or 0)) then
                        ...
                        ImGui.SetCursorPosY(ctx, upperRowY + fontLineFullHeight)
                            ...
                            for i, item in ipairs(app.temp.quickChain) do
                                ...
                                if ImGui.Selectable(ctx, label, false) then
                                    if ImGui.IsKeyDown(ctx, ImGui.Mod_Alt) then
                                        table.remove(app.temp.quickChain, i)
                                    end
                                end
                                if ImGui.BeginDragDropSource(ctx) then
                                ...
                                if ImGui.BeginDragDropTarget(ctx) then
                                    ...
                                    if assetDropped then
                                        local results = app.selection:results()
                                        for j, result in ipairs(results) do
                                            table.insert(app.temp.quickChain, i + j - 1, result)
                                        end
                                    end
                                    if payload then
                                        local oldIdx = tonumber(data)
                                        table.insert(app.temp.quickChain, i, table.remove(app.temp.quickChain, oldIdx))
                                    end
                                    ImGui.EndDragDropTarget(ctx)
                                end
                            end
                ...
                local function handleMenuButtons(rv, btn)
                    if rv then
                        ...
                        elseif btn == 'maximize' then
                            app.settings.current.minimalMode = false
                            app.settings:save()
            ...
            createQuickChainDialog = function(ctx)
                if app.temp.showCreateQuickChainDialog then
                    ...
                    if not ImGui.IsPopupOpen(ctx, title) then
                        ...
                        if not app.temp.quickChainWord then
                            app.temp.quickChainWord = ""
                        end
                    end
                    ...
                    if visible then
                        ...
                        if (ImGui.IsKeyPressed(ctx, ImGui.Key_Enter) and canSaveQuickChain) or app.gui:setting('button', '',
                            ...
                            for _, asset in ipairs(sourceItems) do
                                if asset.key then
                                    table.insert(itemKeys, asset.key)
                                end
                            end
            ...
            mainWindow = function(ctx)
                ...
                local windowFlags = ImGui.WindowFlags_NoTitleBar | ImGui.WindowFlags_NoCollapse
